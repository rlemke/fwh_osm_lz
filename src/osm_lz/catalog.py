...
osm-lz is a pure-FFL workflow catalog: it declares no event facets of its own,
so ``facets()`` is empty here — the facets these workflows compose live in the
sibling ``fwh_osm`` package (see ``osm_geocoder.catalog``). This module mirrors
``osm_geocoder.catalog`` so both packages expose the same accessor surface.

Example::

    from osm_lz.catalog import load_manifest, workflows, facets
...
def facets() -> list[dict[str, Any]]:
    ...
    Empty for osm-lz (a pure-FFL workflow catalog); the composed facets live
    in the ``fwh_osm`` package's manifest.
    """
